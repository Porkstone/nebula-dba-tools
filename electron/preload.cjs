...
contextBridge.exposeInMainWorld("nebula", {
  ...
  postRestoreScript: (name) => ipcRenderer.invoke("post-restore-script", name),
  savePostRestoreScript: (name, script) =>
    ipcRenderer.invoke("save-post-restore-script", name, script),
  ...
  restorePlan: (name, file) => ipcRenderer.invoke("restore-plan", name, file),
